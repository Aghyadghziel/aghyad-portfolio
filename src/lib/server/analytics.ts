...
export async function getDashboard(rangeKey: string | undefined) {
  ...
  const [
    ...
    // My own devices stay in this list — labelled, not hidden — so I can see myself browsing.
    sql`
      with numbered as (
    ...
    sql`
      ...
      select id, visitor_id, nb.visitor_no, nb.visitor_visits,
             ...
             country, region, city, device, browser, os, screen, language, is_new, is_own,
    ...
    sql`
      with active as (
        ...
        select visitor_id, dense_rank() over (order by min(started_at), visitor_id)::int as visitor_no
        from analytics_sessions group by visitor_id
      )
      select a.visitor_id, nb.visitor_no, a.last_seen, t.first_seen, t.visits, t.pageviews, t.seconds,
             f.source as first_source, f.referrer_host as first_referrer_host, f.entry_path as first_page,
             f.utm_source, f.utm_medium, f.utm_campaign,
             l.country, l.region, l.city, l.device, l.browser, l.os,
             coalesce(e.actions, '{}') as actions, m.name as message_name, m.status as message_status
      from active a
      ...
      left join lateral (select array_agg(distinct name) as actions from analytics_events ev
                         where ev.visitor_id = a.visitor_id and ev.type = 'event'
                           and ev.name in ('contact_submit', 'email_click', 'cv_download', 'project_open', 'github_click', 'linkedin_click')) e on true
      left join lateral (select name, status from messages
    ...
    sql`select email, ts, success, reason, ip, country, city, user_agent from admin_logins order by ts desc limit 40`,
    sql`
  ...
  const [source, referrer, campaign, country, city, device, browser, os, screen, language, entry, exit] = breakdowns as Breakdown[];
  ...
  return {
    ...
    series,
    pages,
    realtime: { sessions: realtimeList },
    ...
    admins,
    logins,
    breakdowns: { source, referrer, campaign, country, city, device, browser, os, screen, language, entry, exit },
  };
}
...
/** Everything known about one person: each visit with its source, place and device, and every action in order. */
export async function getVisitorProfile(visitorId: string) {
  await requireAdmin();
  const sql = requireDb();
  if (!VISITOR_ID.test(visitorId)) return null;
  const [number, sessions, events, messages] = await Promise.all([
    sql`
      with numbered as (
        select visitor_id, dense_rank() over (order by min(started_at), visitor_id)::int as visitor_no
        from analytics_sessions group by visitor_id
    ...
    sql`
      select id, started_at, last_seen, extract(epoch from (last_seen - started_at))::int as duration, pageviews, entry_path, exit_path,
             source, referrer, referrer_host, utm_source, utm_medium, utm_campaign,
             country, region, city, timezone, device, browser, os, screen, language, is_new, is_own, own_reason
    ...
    sql`
      ...
      from messages where session_id in (select id from analytics_sessions where visitor_id = ${visitorId})
      order by created_at desc`,
  ]);
  return { visitorNo: (number[0]?.visitor_no as number | undefined) ?? null, sessions, events, messages };
}

/**
 * "This is me" / "Not me" for a browser I never signed in from. Moves every visit that browser
